...
func (s *MySQLStorage) StoreBootstrapToken(r *mdm.Request, msg *mdm.SetBootstrapToken) error {
	_, err := s.db.ExecContext(
		r.Context,
		`UPDATE devices SET nano_bootstrap_token_b64 = ?, bootstrap_token_at = CURRENT_TIMESTAMP WHERE id = ? LIMIT 1;`,
...
func (s *MySQLStorage) RetrieveBootstrapToken(r *mdm.Request, _ *mdm.GetBootstrapToken) (*mdm.BootstrapToken, error) {
	...
	err := s.db.QueryRowContext(
		r.Context,
		`SELECT nano_bootstrap_token_b64 FROM devices WHERE id = ?;`,
		r.ID,
	).Scan(&tokenB64)
	if err != nil {
		return nil, err
